DATA SEGMENT
	...
	INFO3 DB 0DH,0AH,'2.Only Input The Playtime 01~99','$'
	INFO4 DB 0DH,0AH,'3.Only Use +,-,*,/,(,)','$'
	INFO5 DB 0DH,0AH,31 DUP('-'),'$'
...
CODE SEGMENT
	...
	START:
		...
		MOV AH,1				;输入playtime的十位
		INT 21H
		SUB AL,30H
		MOV BH,AL				;保存playtime的十位
		MOV AH,1				;输入playtime的个位
		INT 21H		
		SUB AL,30H
		MOV BL,AL				;保存playtime的个位
		MOV AL,BH
		MOV	CL,0AH
		MUL CL					;十位*10
		XOR BH,BH
		ADD AX,BX				;加上个位，得到100s以内的playtime
		LEA BX,PLAYTIME
		MOV [BX],AL
		MOV AH,0				;获取当前系统时间               
        INT 1AH
		LEA BX,STARTTIME		;保存为游戏开始时间
		MOV WORD PTR [BX],DX
	...
	RESLUT:	
		POP DX					;取计算结果
		CMP DL,18H				;判断结果是不是24
		JNE OVER
		LEA DX,TEXT4			;结果是24，输出游戏成功
		MOV AH,09H
		INT 21H	
		JMP FINISH
		
	OVER:
		LEA DX,TEXT5			;输出TEXT5
		MOV AH,09H
		INT 21H
		MOV AH,0				;获取当前系统时间               
        INT 1AH
		MOV AX,DX
		LEA BX,STARTTIME
		MOV CX,WORD PTR [BX]	;获取游戏开始时间
		SUB AX,CX
		MOV CL,12H
		DIV CL
		LEA SI,PLAYTIME			;获取playtime
		MOV DL,[SI]
		CMP AL,DL				;判断游戏时间是否到了
		JA FAILURE
		JMP INPUT
